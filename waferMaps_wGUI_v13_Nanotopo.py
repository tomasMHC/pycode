...
data_dict = {
    "Item A": ([1, 2, 3, 4], [10, 20, 25, 30]),
    "Item B": ([1, 2, 3, 4], [30, 23, 15, 10]),
    "Item C": ([1, 2, 3, 4], [5, 15, 5, 15]),
}
...
class MainWindow(QMainWindow, Ui_main):
    ...
    def plot_data(self, item):
            ...
            self.canvas.axes.imshow(self.nanotopography(y),cmap="jet",interpolation='spline16',extent=ext)
    ...
    def nanotopography(self,thicknessMatrix):
        ...
        SIGNIFICANCE_LEVEL = int(self.ui.significance_level.displayText())
        ...
        nan_filled_matrix = np.where(np.isnan(nanotopography), np.nanmean(nanotopography), nanotopography)
        z_scores = np.abs(stats.zscore(nan_filled_matrix, axis=None))
        mask = z_scores < SIGNIFICANCE_LEVEL

        filtered_data = np.where(mask,nanotopography,np.nanmean(nanotopography))
